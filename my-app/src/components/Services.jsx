      ...
      {services.map((service) => {
        ...
        return (
          <article key={service.id} className="min-w-0 rounded-lg border border-chamegane p-4 sm:p-5 lg:p-6">
            ...
            <h3 className="break-words font-roboto text-xl font-bold leading-tight text-charcoal min-[380px]:text-2xl">{service.title}</h3>
            <p className="mt-3 break-words font-inter text-sm leading-6 text-charcoal/75 sm:text-[0.95rem]">
              {service.description}
            </p>
          </article>
        )
